...
The coherence-delay law in \cref{sec:tci} concerns estimation with finite
memory. In open-loop settings, the estimator only needs to combine past state
information and new observations. In closed-loop settings, however, the
system's own actions alter future observations. The update must therefore keep
track of what the system did, not only of what it saw.
...
map $U$ whose repeated application preserves drift-coherence must depend on
three arguments:
...
\end{enumerate}
Each argument is indispensable.
\end{lemma}
...
functional decomposition that preserves \cref{cor:ternary} must realize three
roles: a stored state, an action channel, and an adaptation rule that maps the
state, action, and new observation to an updated state. The tuple
