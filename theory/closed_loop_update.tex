...
In closed-loop tracking, the finite-memory law concerns error once a
representation is already in place, but action-effective systems face a prior
bookkeeping question: what must an update rule remember in order to remain
coherent under feedback? In open-loop settings, combining past state and new
observations is enough. In closed-loop settings, however, the system's own
actions alter future observations. The update therefore needs to keep track of
what the system did, not just of what it saw. This point is aligned with
standard action-conditioned belief-state semantics; the role here is to make
that dependency explicit for the diagnostic decomposition, without claiming a
new filtering theorem.
...
map $U$ whose repeated application preserves drift-coherence in this modeling
class must
...
stable or diagnostically meaningful. It only shows that, within the present
action-effective modeling class, a dyadic state-observation update is too small
unless the action term is re-encoded implicitly inside the state. The stability
template that follows supplies the corresponding sufficient conditions.
