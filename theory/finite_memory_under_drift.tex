...
\begin{figure}[!htbp]
\centering
\input{figures/two_clocks_of_drift}
\caption{Two clocks of drift. One clock rewards more memory by reducing
statistical cost; the other penalizes more memory by increasing staleness. The
shaded region is the central operational gap: memory can already be stale
before a detector has enough evidence to alarm.}
...
\begin{figure}[!htbp]
\centering
\resizebox{0.9\linewidth}{!}{\input{figures/lower_bound_witness}}
\caption{Lower-bound witness on the Gaussian location subclass. Two mean paths
separate at the endpoint while remaining hard to distinguish over the retained
window. This is the structural obstruction behind the finite-memory floor: the
window can be too short to estimate reliably and too long to stay temporally
valid.}
