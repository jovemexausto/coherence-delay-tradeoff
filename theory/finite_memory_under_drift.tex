\section{Finite Memory Under Drift}\label{sec:finite_memory_under_drift}

Under continuous drift, memory is both a statistical resource and a temporal
liability. The core question is therefore not whether one can react to change,
but how long a sample should remain operationally useful before it becomes
stale.
...
minimized at the useful-memory horizon $n^*$. The cost crossing occurs later,
so the optimum is about marginal balance rather than equality of levels. The
shaded region marks detector-silent staleness.}
\label{fig:two_clocks_of_drift}
\end{figure}

\subsection{Worst-Case Upper Law}
...
rate is slower than \(n^{-1/2}\). That case is outside the displayed cube-root
regime. This is not a claim of a universal memory law; it isolates the
worst-case linear-staleness envelope studied in this paper.
\end{remark}
...
and vanishes as $\varepsilon\to 0$. We therefore treat Sinkhorn as a calibration
proxy rather than part of the law itself.
\end{remark}
...
memory scale. This is an implementation analogue for exponentially weighted
memory, not a separate general law.
\end{corollary}
