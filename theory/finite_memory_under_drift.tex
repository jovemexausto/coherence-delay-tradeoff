...
structural lower bound, benchmark theorem, and conjectural extensions used in
...
\[
  \mathbb E\left|2\int_0^1 Z_n(u)R_n(u)du\right|
  \le
  2\Bigl(\mathbb E\int_0^1 Z_n(u)^2du\Bigr)^{1/2}
   \Bigl(\mathbb E\int_0^1 R_n(u)^2du\Bigr)^{1/2}
  = o(n^{-1}).
\]
Hence
\[
...
\begin{conjecture}[Fixed-$\varepsilon$ Sinkhorn horizon inheritance]
\label{conj:operational_sinkhorn_horizon}
Assume the fixed-$\varepsilon$ operational setting from
...
results \citep{Stromme2023MID,GroppeHundrieser2024LCA}. In the present embedded
fixed-span model, support-complexity inheritance is exact and the dual
smoothness threshold reduces to $\alpha>k/2$. Calibration supplies the only
remaining ingredient: evidence that the triangular and i.i.d. rate estimates
stay close across the regularization band. That evidence motivates the
conjecture, but it does not close it as a theorem.
