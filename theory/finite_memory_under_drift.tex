...
\caption{Two clocks of drift. The vertical axis is tracking error and the
horizontal axis is the memory horizon, shown in normalized theorem units.
More memory reduces the finite-sample variance term but increases staleness,
producing a U-shaped total error. The optimum $n^*$ occurs before the point
where the two cost levels cross, because the optimum is determined by marginal
balance rather than equality of the two cost values. The shaded region marks
detector-silent staleness.}
\label{fig:two_clocks_of_drift}
\end{figure}

\subsection{Variance-Staleness Decomposition}
...
\subsection{A Structural Worst-Case Horizon}
...
The discussion below uses only that regime.
...
\caption{Lower-bound witness on the Gaussian location subclass. The vertical
axis is the mean shift and the horizontal axis is sample age inside the retained
memory. The two paths are far apart at the present but differ only over a short
recent segment of width $h$, while they are identical over the remaining
retained history. This is the structural obstruction behind the finite-memory
floor.}
