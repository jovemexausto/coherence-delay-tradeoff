...
minimized at the temporal-validity horizon $n^*$. The shaded region marks
pre-detection validity loss: retained evidence can stop representing the present
before enough changepoint evidence accumulates to trigger an alarm.}
...
The only nontrivial assumption in \cref{prop:one_dimensional_root_n_bound} is item~(iv). For
...
full path class, and an embedded Sinkhorn extension in which structural
...
The linear-Gaussian entropic-OT setting is narrower still: its Kalman/Riccati
 structure suggests a constructive intermediate fixed-$\varepsilon$ Sinkhorn setting with an
...
\paragraph{Regular-family horizon law.}
...
\begin{conjecture}[Regular-family horizon law]
\label{conj:regular_family_horizon_law}
