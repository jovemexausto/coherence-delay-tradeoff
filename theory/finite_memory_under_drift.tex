...
Two refinements sharpen this witness story without changing the theorem object.
First, using the exact Gaussian two-point testing error improves the witness
constant relative to the Pinsker-style relaxation while keeping the same
exponent and horizon scale. Second, for $H<1$ the ramp profile is not the
lowest-energy endpoint-saturating H\"older witness; a sharper endpoint-minimal
profile further improves the subclass constant. Those refinements clarify the
constant-level frontier of the main slice, but the paper's central lower-bound
claim remains the structural exponent law stated above.
