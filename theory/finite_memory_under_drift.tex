...
\]
and the corresponding optimal error scale is
\[
...
\cref{cor:optimal_horizon} yields the horizon family
\[
  n^*(a,H) \asymp (C_K/\zeta)^{1/(a+H)}.
\]
...
see \citet{Bahadur1966} and \citet[Chapter~21]{vanDerVaart1998}. The
triangular-array version is taken as an explicit hypothesis of the fixed-span
proof model. One concrete sufficient package is a common compact support
$[L,U]$; rowwise densities $f_{n,j}$ uniformly bounded above and below by
positive constants; a uniform smoothness bound on $f_{n,j}$ across rows; and a
rowwise quantile process whose integrated Bahadur remainder satisfies
...
The lower bound is subclass based rather than class-tight, but it is enough to
show that the horizon is a structural feature of the problem, not an estimator
artifact.
...
These refinements sharpen the constant-level lower-bound picture without
changing the horizon exponents or the status of the main results.
