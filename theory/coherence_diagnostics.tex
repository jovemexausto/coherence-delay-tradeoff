...
\begin{proposition}[Kernel-Level Lower Bound]\label{prop:kernel_lower_bound}
...
satisfies
\[
  \mathbb{E}\,\bigl|\hat\mu_t^{(w)} - \mu_t\bigr|
  \;\geq\;
  c_1\,\sigma\,n_{\mathrm{eff}}(w)^{-1/2}
  + c_2\,\zeta\,\tau_{\mathrm{eff}}(w)
\]
for universal constants $c_1,c_2>0$.
In particular, optimizing over the averaging family yields the same cube-root
exponent as \cref{prop:coherence_delay}.
\end{proposition}
...
n_{\mathrm{eff}}(w)$. For a Gaussian random variable $Z\sim\mathcal{N}(b,v)$,
\[
  \mathbb{E}|Z| \geq \max\!\bigl(|b|,\sqrt{2/\pi}\,\sqrt{v}\bigr)
  \geq \tfrac{1}{2}\bigl(|b| + \sqrt{2/\pi}\,\sqrt{v}\bigr).
\]
Substituting $b=\zeta\tau_{\mathrm{eff}}(w)$ and
$v=\sigma^2 / n_{\mathrm{eff}}(w)$ gives the claim, with constants absorbed into
$c_1,c_2$. Optimizing the lower bound over the averaging family recovers the
same $2/3$ window scaling and $1/3$ error exponent as the upper bound.
\end{proof}
