...
The required window size for target error $\eta$ is therefore controlled by the
same $n^{-1/2}$ versus $\zeta\tau$ trade-off, with intrinsic-dimension and
regularization effects absorbed into $C_K$ rather than the displayed exponent.
...
critical-window regime $1 \leq (\sigma/\zeta)^{2/3} \leq m$: below that range,
the problem is noise-dominated, and above it the available window is too short
for the minimax obstruction to reach the balancing scale.
