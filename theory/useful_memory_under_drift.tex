...
An explicit deformation class makes these conditions verifiable. Let
\[
...
\]
and the same support is preserved. For rowwise mixtures generated by a fixed-span
array of such deformations, the density lower bound and smoothness constants are
therefore explicit functions of the deformation radius $\Delta$, while the
within-window drift span enters only through the deformation amplitudes. On the
tested smooth fixed-support family, the integrated residual decays at empirical
rates between $1.40$ and $1.54$ for spans $0.2$ and $0.4$, and the scaled
quantity $n\int_0^1 R_n(u)^2du$ still decays at rates between $0.40$ and $0.54$;
see \cref{app:finite_sample_geometry}. On the fixed-span triangular-array
...
do not replace a general verification theorem, but they make the proof-model
assumption explicit on two common deformation families and quantify its span
dependence on the classes currently used in the manuscript.
