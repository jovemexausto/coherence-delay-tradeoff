\section{Temporal-Validity Horizon and Useful-Memory Geometry}
...
$P_t$ combine into a horizon law. When both sides are controlled, the resulting scale
...
\]
The case $H=1$ is the local Lipschitz envelope; smaller $H$ corresponds to slower
...
\subsection{One-Dimensional Root-$n$ Proof Model}
...
\subsection{Verifiable Conditions for the 1D Proof Model}
...
a package, but no general dependent-row
theorem.
\end{remark}
...
\subsection{Finite-Family and Sliced Transfer}
...
\subsection{Benchmarks and Sinkhorn Geometry}
...
\begin{theorem}[Finite-discrete embedded moderate-band Sinkhorn inheritance]
...
\begin{proposition}[Covariance-weighted support-growth null control]
...
\begin{corollary}[Local block-curvature criterion for support growth]
...
\begin{proposition}[Uniform parity-channel curvature bound]
...
\begin{theorem}[Calibrated quadratic support-growth Sinkhorn band law]
...
Beyond the one-dimensional and bounded-cardinality finite-discrete band results above, assume the fixed-$\varepsilon$ Sinkhorn setting from
...
GroppeHundrieser2024LCA}. In the embedded fixed-span model considered here,
...
fitted exponents can agree closely on moderate fixed-$\varepsilon$ bands.
On the calibrated discrete $k=2$ support grid, the centered squared
self-coupling block keeps a uniform gap on the moderate band, the exact-target
null slopes stay near the quadratic remainder scale, the exact
$\ell_2$ fluctuation is $3/(4n)$, and the empirical cost-to-$\ell_2^2$ ratios
stay stable across the reported support-growth grid. This reduces the support-
growth extension problem to a uniform quadratic null bound on the calibrated
family and, beyond that, to support change beyond the fixed support-growth
construction.

Regular dominated families remain the natural family-level extension class.
Support-changing and singular families fall outside the present regime.

Once the finite-sample regime and the roughness level are fixed, the law yields
the memory scale and the corresponding error scale. Rougher drift and weaker
finite-sample rates imply shorter memory and larger residual error.
