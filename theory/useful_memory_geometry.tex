...
\begin{enumerate}
  ...
  \item the proof is tested on the Gaussian location subclass
  $X_t \sim \mathcal N(\mu_t,\sigma^2 I_d)$ with
  $\|\mu_t-\mu_s\|_2 \le \zeta |t-s|$;
  \item the estimator at time $m$ only uses the last $m$ samples.
\end{enumerate}
Measure risk by the endpoint $W_2$ error (equivalently, Euclidean endpoint
error in the Gaussian location subclass). Then, in the critical-window regime,
the minimax tracking error over that class cannot beat the same
...
Gaussian location subclass whose mean path is itself $W_2$-Lipschitz. That is
enough to make the result minimax for any larger class containing that subclass,
including the paper's general $W_2$-Lipschitz path class. So the proposition is
not claiming that every path is Gaussian; it is claiming that the worst case
already appears inside the Gaussian mean-shift subproblem.
\end{remark}
