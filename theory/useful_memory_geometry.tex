...
\begin{itemize}
  ...
  $(\log n)^{1/2}$ factor.
	...
	$n^{-1/2}$ carrier exponent, so the resulting horizon scaling becomes
	carrier-dependent (see \cref{rem:carrier_exponent}).
...
\begin{remark}[Carrier-dependent horizon scaling]\label{rem:carrier_exponent}
...
rate is slower than \(n^{-1/2}\), so the exponent is strictly smaller than
\(1/2\) (carrier-dependent). The cube-root law is therefore the
\emph{$a=1/2$ carrier instance} of a broader carrier-dependent horizon
scaling.
\end{remark}
