...
and filter-bubble-like drift more clearly than passive detectors and a
...
entropy surrogate is the intended operational choice. The logged benchmark is
evidence for a useful proxy, not a proof that the surrogate equals the
unobserved conditional entropy. KuaiRand is a proxy-level benchmark.
...
distance may be used as a fallback, but KL is the preferred default. In the
follow-up proxy sweep, the KL proxy also outperformed the total-variation and
exposure-Gini alternatives at the same healthy-only calibration, so it remains
the default operational choice here.
We use additive smoothing $10^{-6}$ in the KL computation.

\subsection*{Protocol}
The benchmark unfolds in three phases.
...
batched across users with strict temporal splits. Thresholds for CI,
CI$^E$, and EWMA-smoothed CI$^E$ are calibrated on healthy windows only,
...
\begin{itemize}[leftmargin=*]
  ...
  \item sensitivity to $\lambda$ in CI$^E$,
...
$1 - CI^E$, so the comparison is not biased by detector-specific
...
keep the benchmark from collapsing into a hindsight-tuned logged-bandit
