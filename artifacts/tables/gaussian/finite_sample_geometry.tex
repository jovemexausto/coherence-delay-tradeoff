...
\centering
\small
\caption{Empirical scaling diagnostic for the finite-sample term. The bounded-support i.i.d. $W_2$ experiment is close to the expected $n^{-1/2}$ rate in low dimension, the corresponding triangular-array diagnostic compares one sample from each drifted component against an i.i.d. sample from the window mixture, and the Sinkhorn diagnostic is reported at fixed $\varepsilon$. These rows are numerical support for the operational finite-sample discussion rather than theorem statements.}
