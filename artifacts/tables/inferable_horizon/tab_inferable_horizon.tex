\begin{table}[!htbp]
\centering
\caption{Representative inferable-horizon regimes across the operational identifiability transition.}
\label{tab:inferable_horizon}
\begin{tabular}{rrrrrrr}
\toprule
$L$ & $m$ & $H$ & $\delta$ & $\mathfrak S_{\delta,m,L}$ & Empirical & Gaussian \\
\midrule
5 & 20 & 0.3 & 0.050 & 0.501 & 0.367 & 0.384 \\
8 & 10 & 0.9 & 0.020 & 1.001 & 0.664 & 0.683 \\
3 & 200 & 0.3 & 0.010 & 1.996 & 0.949 & 0.954 \\
8 & 100 & 0.3 & 0.005 & 3.001 & 0.997 & 0.997 \\
\bottomrule
\end{tabular}
\end{table}
