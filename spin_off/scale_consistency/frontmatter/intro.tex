...
governs estimation of $H$, residual adequacy testing, the minimax separation
...
exact oracle null law with a feasible asymptotic counterpart.
...
\item \textbf{One adaptive procedure.} The same test attains the boundary
without prior knowledge of $H$.

\item \textbf{One unknown-scale extension.} A decoupled Gaussian benchmark for
unknown $\sigma_0$ yields an exact split-$F$ null law, while same-sample
parametric bootstrap restores near-nominal calibration on the tested grid.

\item \textbf{Three empirical signatures.} The simulations show null
calibration, FWLS-oracle agreement, and a boundary transition aligned with the
information scale.
\end{enumerate}
...
adaptive testing connect through \citep{dahlhaus1997fitting,
...
Section~\ref{sec:test} establishes the residual adequacy law and its response
