...
\centering
\small
\caption{Two-step feasible weighted least squares procedure.}
\label{tab:fwls-procedure}
\resizebox{\linewidth}{!}{%
\begin{tabular}{p{0.18\linewidth}p{0.74\linewidth}}
\toprule
Input & $(\Dhj)_{j=1}^L$, $n$, and $\sigma_0$. \\
\midrule
Step 1 & Compute $(\hat\alpha_{\mathrm{pil}}, \hat H_{\mathrm{pil}})$ by unweighted OLS of $\log \Dhj$ on $(\mathbf{1}, \log j)$. \\
Step 2 & Form the pilot fitted profile $\hat D_j^{\mathrm{pil}} = \exp(\hat\alpha_{\mathrm{pil}} + \hat H_{\mathrm{pil}} \log j)$. \\
Step 3 & Set $\hat w_j = n (\hat D_j^{\mathrm{pil}})^2 / \sigma_0^2$ and compute $(\hat\alpha, \Hhat)$ by WLS with weights $(\hat w_j)$. \\
Output & Return $\Hhat$, $\hat\alpha$, and residuals $\hat U_j = \log \Dhj - \hat\alpha - \Hhat \log j$. \\
\bottomrule
\end{tabular}%
}
\end{table}
