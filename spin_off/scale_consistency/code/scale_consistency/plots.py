...
def save_fwls_oracle_gap_figure(rows: list[object], output_path: Path) -> None:
    ...
    ax.set_title("FWLS-oracle agreement")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(output_path)
    plt.close(fig)
...
def save_boundary_power_figure(rows: list[object], output_path: Path) -> None:
    ...
    ax.set_title("Power near the minimax boundary")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(output_path)
    plt.close(fig)
...
def save_rate_constant_figure(rows: list[object], output_path: Path) -> None:
    ...
    ax.set_xscale("log")
    ax.set_xlabel("n")
    ax.set_ylabel("Scaled constant")
    ax.set_title("Rate-constant stabilization")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(output_path)
    plt.close(fig)
...
def save_robustness_figure(
    ...
    ax.set_title("Scale misspecification")
    ax.set_ylim(0.0, 1.05)
