...
- Manuscript state: the main text now follows `object -> law -> proof anchor -> transfer / Sinkhorn theorems -> horizon inference -> empirical signatures -> related work -> limitations -> conclusion`. Section and file names have been normalized to that order, the last open Sinkhorn conjecture has been moved out of the theorem line, and the main arc now reaches `Related Work` by page 14 with `Conclusion` starting on page 15.
- Introduction compression: the fivefold contribution list has been replaced by a short contribution summary in the intro, and the detailed contribution map now lives in the appendix to free space at the top of the paper.
- Introduction compression: the fivefold contribution list has been replaced by a short pointer sentence in the intro, and the full five-item contribution summary now lives at the end of the appendix so it does not compete with the appendix floats or the top-of-paper space budget.
- Introduction compression: the fivefold contribution list has been replaced by a short pointer sentence in the intro, and the full five-item contribution summary now lives at the end of the appendix so it does not compete with the appendix floats or the top-of-paper space budget.
- Prose compression: the intro now runs as a short object-first statement of the law, structure, and theorem line; section openings in horizon inference, empirical signatures, the main horizon-law section, and limitations have been shortened to reduce explanatory bulk.
...
- Verification status: the root manuscript compiles via `tectonic`; the main arc now starts `Empirical Signatures` on page 12, `Related Work` on page 14, `Limitations` on page 14, and `Conclusion` on page 15.
<!-- PROJECT_STATE_END -->
