...
- Closed theory: upper law; useful-memory geometry and exact optimizer; exact uniform-window staleness constant; 1D root-`n` proof model with Bahadur control; Gaussian lower benchmark and location benchmark; finite-family projection / sliced / product transfer; deterministic inferable-horizon stability and regret control; 1D fixed-`epsilon` Sinkhorn inheritance; conditional embedded moderate-band Sinkhorn proposition on the calibrated `k=2` grid; weighted-memory identities.
- Empirical layer: U-curves, roughness scaling, validity-before-detection gap, real-stream ELEC2 interior optima, online controller separation, and structured Sinkhorn calibration bands. Current evidence supports the calibrated `(8,2)` / `(12,2)` moderate-band Sinkhorn regime and the observation-based validity gap, while residualized detectors are not uniformly better.
- Active theorem program: three fronts remain open. `notes/12-theorem-ladder.md` and `notes/13-sinkhorn-self-coupling-lemma.md` now isolate the exact missing Sinkhorn self-coupling lemma; the faster backup is finite-family projection/product-`W_2`; the cheap bridge is deterministic inferable-horizon control.
- Root-code audit: reproducible sweeps and manifests now exist for inferable horizons, ELEC2, validity gap, online control, and Sinkhorn calibration. The remaining technical debt is the Sinkhorn backend, which is still not log-domain/cached.
- Immediate next steps: finish the paper-edit pass for the 15--20 page contract; compress the front matter and main theorem line; keep the Sinkhorn lemma as a conditional proposition unless proof progress arrives; decide whether the next build cycle pursues the self-coupling lemma or stops at the current theorem-facing evidence.
- Verification status: the current root manuscript compiles via `tectonic`; targeted tests for inferable horizons, robustness sweeps, online control, and Sinkhorn calibration pass.
<!-- PROJECT_STATE_END -->
