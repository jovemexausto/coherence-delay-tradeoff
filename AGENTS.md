...
- Archive restructuring: the main text follows the arc `object -> law -> proof model -> horizon inference -> transfer/operational questions -> empirical signatures -> related work -> limitations -> conclusion`, with section titles and local terminology pushed toward more direct scientific naming, a prose-first abstract, an explicit enumerated contribution block organically integrated into an object-first introduction, weighted-memory material deferred out of the opening theorem line, no roadmap-style introduction ending, a shorter pre-contribution introduction and shorter limitations/closing arc, lighter metatext in status language, more compressed section exits, cleaner float/caption discipline, and an enumerated open-problem frontier. The contribution block now uses a shorter embedded-Sinkhorn status line to protect page flow.
