...
- Current manuscript identity: the spin-off is centered on a single object, `structural inference for a power-law lag geometry under signal-coupled heteroscedastic noise`, equivalently `an information law for structural inference on lagged transport geometry`. The organizing law is `n\sum_{j\le L} j^{2H}`: it governs estimation, testing, lower bounds, and adaptivity as one inferential scale. The main-text architecture is now fixed as `Introduction -> Model -> Inference -> Optimality -> Simulation Evidence -> Operational Implication -> Limitations and Open Problems -> Conclusion`, with the body reduced to one FWLS theorem, one null-law theorem, one alternative-response proposition, one lower bound, one adaptivity theorem, three figures, one small procedure table, and a separate structured limitations section; the appendix carries full grids, robustness, constant derivations, and auxiliary proofs. The preferred title direction is short and object-first, such as `Power-Law Lag Geometry: Structural Inference from Noisy Transport Estimates`. Existing theorem and artifact work should now be judged against this framing, with any section, figure, or table retained only if it serves this single identity.
