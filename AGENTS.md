...
- Archive restructuring: the main text follows the arc `object -> law -> proof model -> inferable horizons -> extensions/status -> empirical signatures -> related work -> limitations -> conclusion`, with a dedicated `Extensions and Operational Status` section, a prose-first abstract, weighted-memory material deferred out of the opening theorem line, a shorter limitations/closing arc, lighter metatext in status language, and cleaner float/caption discipline.
