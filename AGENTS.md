...
- The paper/appendix/repository split is now partially implemented: nonessential operational mechanics and family-level support figures have been removed from the main text, a short `Repository Guide` appendix points to provenance in the repo, and finite-sample calibration detail is explicitly subordinated to appendix/repository status.
