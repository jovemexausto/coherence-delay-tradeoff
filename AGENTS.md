...
- Empirical layer: U-curves, roughness scaling, ELEC2 interior optima, validity-before-detection lag, structured Sinkhorn calibration bands, and calibrated support-growth diagnostics. Cheap operational ablations now supplement the main signatures: online memory-selection sweeps across `default/smooth/rough/alternating/ramp_up` profiles show structural controller ratios around `1.18`--`1.36` of oracle risk versus `2.30`--`4.41` for the raw plug-in rule and `1.34`--`1.65` for the activity baseline; detector/input robustness tables already show positive validity gaps across observation-based ADWIN/Page--Hinkley grids and residual-input failure modes; weight-sensitivity diagnostics compare uniform, triangular, and geometric tapers through `n_eff`, mean lag, and horizon proxies.
- Empirical layer: the operational-identifiability figure has been rewritten from an overplotted scatter into binned hit-rate curves with uncertainty bands, so the score transition is now the primary visual signal rather than dense point clouds.
...
- Prose compression: the intro now states the object, law, dimensional role of geometry, and theorem/empirical arc without inventory prose; section openings in horizon inference, empirical signatures, the main horizon-law section, and limitations have been shortened. The real-stream and moderate-band Sinkhorn diagnostics now state the main signal in the body and leave calibration detail to the appendix. The empirical summary now references online-controller baselines, and transfer/operational implications now point to weight-sensitivity diagnostics.
- Scope/framing cleanup: the lower-bound discussion now distinguishes the subclass structural lower result from the closed Gaussian-location benchmark, the limitations section now enumerates only four genuine open problems in the present paper line, and the conclusion no longer mixes those open problems with the broader research agenda kept in `notes/14-checkpoint.md`.
...
- Root-code audit: reproducible sweeps and manifests remain for inferable horizons, ELEC2, validity gap, online control, Sinkhorn calibration, and support-growth Hessian probes. Targeted tests for online adaptation, detector-gap robustness, and weight sensitivity pass. The compile still shows the pre-existing `algorithm.sty` UTF-8 warning outside the edited sections.
...
- Checkpoint discipline: `notes/14-checkpoint.md` now splits paper-completion debt (the `S^2` lemma and embedded influence-class item (iii)) from genuine frontiers and from the broader research agenda, keeping the paper line clean.
...
- Verification status: the root manuscript compiles via `tectonic`, now fits in 20 pages total, places the conceptual horizon figure in the middle of the introduction on page 1 immediately after the sentence introducing the temporal-validity horizon and useful-memory region, restores the appendix contribution-summary list, starts `Empirical Signatures` on page 12, `Related Work` on page 13, `Limitations` on page 14, and `Conclusion` on page 15. Figure 3 has been redrawn as binned identifiability curves with uncertainty bands and the surrounding text now emphasizes the score transition instead of raw point overlap.
<!-- PROJECT_STATE_END -->
