...
<!-- PROJECT_STATE_BEGIN -->
Project status:
- Core object: temporal-validity horizon for drifting distribution tracking. Main law: `E d(\widehat P_t^{(n)},P_t) <= C_K n^{-a} + C_S zeta n^H`, now framed as the constitutive validity law; the optimized value is the temporal-validity functional `V(\phi,\tau)`; optimal scale `n^*(a,H) ~ (C_K/zeta)^{1/(a+H)}`; useful-memory region `U_delta` is controlled by `Psi(x)=(H x^{-a}+a x^H)/(a+H)`.
- Closed theory: abstract upper law; useful-memory geometry and exact optimizer; exact uniform-window staleness constant; 1D proof anchor with Bahadur control; Gaussian lower benchmark and Gaussian-location benchmark; finite-family projection / sliced transfer; validity-observability CLT, identifiability, deterministic stability, and regret control; bounded-cardinality finite-discrete embedded Sinkhorn horizon law; calibrated support-growth Sinkhorn horizon law via exact `\ell_2` fluctuation identity, exact row-kernel parity decomposition, and uniform parity-channel curvature bounds.
...
- Submission polish: abstract, introduction, transfer, related work, limitations, and conclusion now distinguish proved results from effective-exponent interpretation without metatext; the current framing foregrounds the temporal-validity functional `V(\phi,\tau)` as a persistence field over inferential structures and flow. The separability step is now an auxiliary lemma, and the Gaussian location minimax benchmark is the leading theorem in the horizon section. The transfer section gives practical guidance for choosing projection families or slicing laws; the Bahadur remark states explicitly that the smooth deformation class is the verified instance; the validity-observability section uses a compact `algorithm`/`algpseudocode` pipeline and names `aH` as the local validity curvature; the introduction now closes on inferential persistence rather than memory choice; the related-work section now positions the paper against Watanabe's static equations of state, Huang--Wang's stability principle, Kim's metastability picture, and distributional AoI; the empirical summary and online-adaptation discussion now compare the structural controller against adaptive EWMA and scalar Kalman baselines, and the validity-lag subsection now states the calibrated `H<1` detector frontier directly.
- Figure state: the binned hit-rate operational-identifiability figure remains the main detector view; the introduction now includes a validity-field figure over the $(a,H)$ plane, rendered via `uv` from the project environment and resized upward after the structure reordering.
...
- Verification status: the root manuscript compiles via `tectonic`, now spans 21 pages total after the validity-field figure and structural reordering, and still shows the pre-existing `algorithm.sty` UTF-8 warning outside the edited sections. The conclusion has been moved ahead of the related-work and limitations material so the main line can be read first.
- Final polish: the last remaining mechanical phrasing in the transfer, validity-observability, introduction, and conclusion sections has been smoothed without changing claims or content; the submission build remains at 20 pages.
- Final polish: the conclusion now closes on the measurable temporal-validity profile of retained statistical structure and its critical persistence scale rather than repeating an operational tuning message; the submission build remains at 20 pages.
...
- Repository layout: the follow-up scaffold `projects/sequential-validity-detection-delay/` now exists with a seeded README, a minimal TeX tree, and a working-plan note that records the detector-class theorem target; its abstract, introduction, and main-law scaffold now formalize the detector class via the finite-horizon false-alarm budget on the stationary null, state the benchmark Gaussian sign result, and keep the stronger contiguity-based delay lower bound as open. The main package now also contains false-alarm-calibrated sequential-delay frontier code and CSV output under `artifacts/csv/invalidity_gap/calibrated_delay_frontier.csv`.
<!-- PROJECT_STATE_END -->
