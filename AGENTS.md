...
<!-- PROJECT_STATE_BEGIN -->
Project status:
- Compact current state: the paper centers on the temporal-validity horizon `V(\phi,\tau)` and the law `E d(\widehat P_t^{(n)},P_t) <= C_K n^{-a} + C_S \zeta n^H`, with lag-geometry inference, robust bootstrap calibration, regime-dependent operational control, and a new conformal-calibration branch under drift.
...
- Core object: temporal-validity horizon / validity field `V(\phi,\tau)` for inferential persistence under flow, with the Holder--Wasserstein law `E d(\widehat P_t^{(n)},P_t) <= C_K n^{-a} + C_S \zeta n^H`, optimizer `n^*(a,H) ~ (C_K/\zeta)^{1/(a+H)}`, and operational loss `\mathcal L_t(n) + c_t^\pi` when memory is reconfigured.
