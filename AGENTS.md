...
- Current manuscript identity: the spin-off owns structural inference for a power-law lag curve from noisy transport estimates, with object `(H, zeta)` in `D_j=zeta j^H` and inferential scale `I_{n,L}(H)=n\sum_{j\le L} j^{2H}`. Its theorem core is FWLS oracle-equivalence, exact oracle `\chi^2(L-2)` adequacy with a feasible asymptotic counterpart, variance-inflation response under departures, a Le Cam separation lower bound, adaptive attainment for `L=L_n`, and a decoupled unknown-`sigma_0` split-`F` benchmark; its empirical core is null calibration, boundary power, FWLS/oracle agreement, and bootstrap-vs-split calibration diagnostics. Boundary with the root paper: this manuscript is about inference on lag geometry from estimated discrepancies, not about optimal memory under drift, useful-memory regions, validity-detection lag, or Sinkhorn/Wasserstein horizon transfer.
