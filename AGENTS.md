...
- Current manuscript identity: the spin-off owns structural inference for a power-law lag curve from noisy transport estimates, with object `(H, zeta)` in `D_j=zeta j^H` and inferential scale `I_{n,L}(H)=n\sum_{j\le L} j^{2H}`. Its theorem core is FWLS oracle-equivalence, the joint asymptotic law of `(\hat\alpha,\hat H)` with a smooth-functional corollary, exact oracle `\chi^2(L-2)` residual law with a feasible asymptotic counterpart, variance-inflation response under departures, a Le Cam separation lower bound, adaptive attainment for `L=L_n`, and a decoupled unknown-`sigma_0` split-`F` benchmark; its empirical core is oracle law calibration, boundary power, FWLS/oracle agreement, and bootstrap-vs-split calibration diagnostics. Boundary with the root paper: this manuscript is about inference on lag geometry from estimated discrepancies, not about optimal memory under drift, useful-memory regions, validity-detection lag, or Sinkhorn/Wasserstein horizon transfer.
...
- Next-cycle plan: first close embedded fixed-`epsilon` Sinkhorn on a moderate band via direct linearization, uniform self-coupling stability, and a triangular-array delta-method transfer; then pursue a multivariate transfer lemma for smooth/Hadamard-differentiable functionals, with a toy moment-vector-plus-smooth-map check as the executable proxy for the Jacobian/remainder structure; in parallel harden Bahadur sufficient conditions, add theorem-oriented Python tests for non-uniform weights via `n_eff` and lag-moment summaries, and keep online adaptation at plug-in containment level. For the spin-off, the immediate V1 track is: lock the standalone paper architecture around model/test/lower-bound/adaptivity, replace broad literature inventory with theorem-relevant citations only, and build a Python package that reproduces null calibration, boundary power, FWLS-oracle equivalence, and sensitivity to misspecified scale/noise regimes. The bridge track for the root paper is now explicit: use the PLLG joint law for `(\hat\alpha,\hat H)` to derive plug-in horizon CLTs, useful-region coverage, and operational regret through the inferable-horizon experiments. Validation-detection theory remains last unless a detector-specific theorem appears cheaply. External review aligned with the current identity and highlighted two concrete edits: split the asymptotic discussion cleanly into fixed-`L` versus `L\to\infty` regimes, and distinguish exact oracle null laws from asymptotic validity when weights are estimated.
- Verification status: core theorem and weighting tests currently pass (`26` Python tests in the expanded verification sweep), the weighted-memory identities and the new effective-sample-size proposition match their closed forms on linear ramps, the certified-band report is positive on the calibrated Sinkhorn grid, the self-coupling certificate is positive on the calibrated moderate-band pairs, the multivariate transfer diagnostic has an interior U-curve with a lower-order linearization residual, and the current Sinkhorn certificate remains positive on the calibrated synthetic grid. The spin-off workspace compiles cleanly via `tectonic`; the latest rebuild after the framing, bibliographic, limitations, and plug-in update is clean, and the body now contains only the three intended figures plus the single FWLS procedure table and a dedicated limitations section. Recent editorial tightening also retitled the paper and internal sections around structural inference, residual-based adequacy, separation boundaries, and information-normalized error summaries; restored a compact relation-to-prior-work paragraph with citations on roughness estimation, Wasserstein limits, locally stationary drift, adaptive testing, and cross-citation to the companion preprints `Parreira (2026a, 2026b)`; and shortened the abstract, introduction, model remark, and conclusion to a cleaner one-law framing. Under `uv run`, the local Python package includes `model`, `estimation`, `theory_diagnostics`, `experiments`, `report`, and `plots`, passes `22` unit tests, and the generated CSV/TEX/PDF artifacts under `spin_off/scale_consistency/artifacts/` now include a sigma0 calibration table showing a three-way separation: naive self-normalized `\chi^2` calibration collapses to near-zero rejection, the oracle split-`F` benchmark remains near nominal, the fully feasible split-`F` procedure is conservative, and parametric bootstrap calibration restores near-nominal size on the tested grid. In the root workspace, the new `inferable_horizon.py` bridge module runs under `uv`, exports CSV and figure artifacts, and the initial grid shows: joint-FWLS covariance matches theory closely, plug-in `\log \hat n^*` standard deviations match delta-method predictions, useful-region hit rates collapse against `\sqrt{\mathcal I} r_\delta / \tau`, and empirical relative regret matches the quadratic prediction `(aH/2)\tau^2/m`. The root manuscript now contains a dedicated inferable-horizons section with a plug-in horizon CLT, an operational-identifiability theorem, a quadratic regret proposition built on the useful-memory region geometry, and a representative transition table plus figure calibrated to non-saturated identifiability regimes. Recent editorial tightening also retitled the paper and internal sections around structural inference, residual-based adequacy, separation boundaries, and information-normalized error summaries; restored a compact relation-to-prior-work paragraph with citations on roughness estimation, Wasserstein limits, locally stationary drift, adaptive testing, and cross-citation to the companion preprints `Parreira (2026a, 2026b)`; pruned duplicate bibliography keys to a single lowercase set; moved the title block to the same short-title plus explanatory-subtitle pattern as the root paper; restored the FWLS procedure as a boxed table after removing the problematic algorithm package; added explicit `cleveref` names for theorem-like environments so assumption references now resolve correctly; inserted a section roadmap in the introduction for stronger narrative flow; suppressed proof-end square markers that were reading as PDF artifacts; and eliminated the previous `algorithm.sty` UTF-8 warning together with the duplicate-equation-object warning. The current spin-off artifacts now back the corrected appendix constant `C\approx 2.22`, report FWLS/oracle RMSE ratios numerically at `0.9998--1.0007`, replace the stale hand-written appendix tables with generated tables, remove the previous fixed-`L` non-central-`\chi^2` attainment language in favor of a covariance-inflation interpretation plus a growing-lag consistency theorem, and eliminate the previous Python runtime warning from eager `plots` imports.
- Open blockers: embedded bandwise Sinkhorn inheritance beyond the compact-support one-dimensional closure; sharp lower theory beyond the current benchmark family and root-$n$ regime; verified bounded-support Bahadur remainder classes; finite-sample calibration theory for the plug-in `sigma_0` path; online useful-memory adaptation with guarantees; validity-detection theory. The tapered-weight diagnostic and theorem-style tests now isolate a credible non-uniform-memory law candidate via `n_eff` and lag moments, but whether it supports a robust theorem or sharper constants remains open.
<!-- PROJECT_STATE_END -->
