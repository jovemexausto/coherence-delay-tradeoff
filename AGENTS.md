...
<!-- PROJECT_STATE_BEGIN -->
Project status:
- Core object: temporal-validity horizon for finite-memory tracking under drift, with law `E d(\widehat P_t^{(n)},P_t) <= C_K n^{-a} + C_S zeta n^H` and optimizer `n^*(a,H) ~ (C_K/zeta)^{1/(a+H)}`.
...
- Operational status: fixed-`epsilon` Sinkhorn is theorem-level in the 1D compact-support class and conjectural in the embedded/high-dimensional bandwise class.
...
- Empirical essentials: U-curve, roughness scaling, moderate-band Sinkhorn stability, validity loss before detector alarm.
<!-- PROJECT_STATE_END -->
