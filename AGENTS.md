...
- The empirical Sinkhorn section now avoids an explicit operational-map presentation and states the bandwise evidence directly in prose.
- Related work now explicitly contrasts the horizon law with resource-aware update scheduling under drift (RCCDA), streaming OT-based change-point detection, and efficient MMD-based detectors (MMDEW) as neighboring age/variance trade-off problems with different objectives.
- The related-work contrast has been tightened to emphasize objective-function differences: RCCDA concerns policy feasibility under budget, OT/MMD detectors concern alarm timing and power, and the horizon law concerns when retained evidence itself becomes stale.
- The related-work section now uses angle-based framing consistently: comparator regret, detector alarms, policy feasibility, latent-state tracking, local-function smoothing, proxy geometry, regularized OT, and AoI are each positioned by the quantity they optimize or measure, not by a generic "adjacent but distinct" contrast.
- The 1D proof-model remark now gives explicit sufficient-condition examples for the triangular-array Bahadur remainder: common compact support, densities bounded away from zero with uniform smoothness, and bounded smooth monotone deformations or support-preserving fixed-support location perturbations; the general non-i.i.d. verification theorem remains open.
- The 1D proof-model remark now states a more concrete sufficient-condition package for the triangular-array Bahadur remainder: common compact support, rowwise densities uniformly bounded above and below with uniform smoothness, and an integrated remainder $o(n^{-1})$; the non-i.i.d. verification theorem remains open.
- The title/subtitle line is now aligned across manuscript and repository metadata as `Useful Memory Has a Horizon: Temporal Validity of Retained Evidence under Drift`, matching the paper's central object more directly than the older structural-limits wording.
...
- The abstract opening no longer repeats the title's drift framing in its first words; it now starts directly from the memory trade-off before introducing temporal validity under drift.
- The abstract middle no longer inventories proof mechanics; it now states only the main closed regimes, the Sinkhorn theorem/conjecture boundary, and the final takeaway.
- Internal proof notes were normalized to match the manuscript notation: plain-text `int_0^1`/`C_epsilon` shorthand was replaced with the corresponding LaTeX forms, and the ramp/endpoint Gaussian lower-bound constants now use the same `\mathrm{ramp}` / `\min` conventions as the appendix and main text.
