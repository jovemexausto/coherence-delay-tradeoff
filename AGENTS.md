...
- Current manuscript identity: object-first theory of temporal validity under drift, with a coherent sequential arc now ordered as theory, empirical signatures, implications for online adaptation, related work, limitations, and conclusion; related-work framing is rewritten around mutual contribution rather than topic contrast, now including an explicit tracking-versus-DRO distinction relative to `KeehanAndersonWiesemann2025`, and appendix language mirrors the same object-first tone. Verifiable 1D sufficient conditions now sit in their own subsection, the multivariate sliced/product extension now follows the lower-bound/benchmark line, empirical subsection labels and captions are normalized for consistent navigation, the bibliography has been pruned to cited/useful entries only, and the limitations section now organizes five open problems so that Problems 1--2 close the current theorem line, Problem 3 isolates memory design, and Problems 4--5 define the subsequent online theory program. A spin-off paper on power-law lag geometry under noisy transport estimates is now consolidated as a standalone structural-inference paper rather than a testing-only paper, with adequacy testing treated as one component of inference, application language reduced to a short operational implication, all LTSS wording removed, and a lighter editorial arc that keeps the main theorem line in the text while pushing numerical tables and low-yield side remarks to the appendix; its workspace now lives under `spin_off/scale_consistency/` with local `config/`, local `bibliography.bib`, local `pyproject.toml`, local `.venv`, and dedicated `code/`, `tests/`, `artifacts/`, and `notes/` directories. External feedback also reinforced the spin-off identity as structural inference for a power-law lag geometry under signal-coupled heteroscedastic noise, with the most useful near-term edits being scope sharpening, regime separation, theorem-relevant citation discipline, scientifically sharper titles/labels, and a root-paper-style editorial pass on section flow. The highest-value latent theorem pattern in the spin-off is now explicit: a unified information law driven by the weighted lag-energy `\sum_{j\le L} j^{2H}` controls oracle variance, residual-test noncentrality, and the minimax boundary, with the manuscript and Python diagnostics rewritten around `n\sum_{j\le L} j^{2H}` so that fixed-`L` and growing-`L` appear as two regimes of one scale law.
