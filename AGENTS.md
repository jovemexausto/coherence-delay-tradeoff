...
- A new TwoNN operational diagnostic on the synthetic frontier gives a stronger, question-specific result: on the 9-pair / 6-epsilon grid, `k_hat` beats `ambient_dim` on leave-one-out MAE for `epsilon_max` and on the pair-level stability cut at `epsilon=0.2`; the full rowwise stability map remains mixed, so TwoNN is a useful geometry proxy but not a universal resolver of the frontier irregularity.
...
- The manuscript now carries TwoNN naturally as related-work support for local intrinsic-dimension estimation and as an empirical regime-selection proxy in the operational frontier section; the citation is Facco et al. (2017), "Estimating the intrinsic dimension of datasets by a minimal neighborhood information" in Scientific Reports.
