\section{Horizon Inference from Lag Geometry}
\label{sec:validity_observability}

Structural validity is not enough for action. A data-driven information-state
rule needs estimators of the drift parameters $(H,\zeta)$ together with a
criterion that compares inferential resolution to the curvature of the
useful-memory region. Lag geometry supplies that inference surface: present-time
validity becomes observable when discrepancies across controlled lags reveal the
roughness and amplitude of the underlying drift.
...
\begin{assumption}[Lag-geometry observability input]
...
\begin{theorem}[Plug-in Validity Central Limit Law]
...
\begin{theorem}[Operational Validity Identifiability of Useful Memory]
...
\]
is the operational identifiability score of the horizon. It compares
lag-geometry information to the validity-curvature-imposed precision requirement of the
useful-memory region.

The following algorithm summarizes the operational pipeline.
\begin{algorithm}[!htbp]
\caption{Operational validity-observability pipeline}
...
The lag-geometry pipeline is accurate on the power-law family used in the
assumption, but it is not stable under arbitrary departures from that family.
Targeted Monte Carlo checks on sinusoidal and piecewise departures show a small
bias under the exact model and a much larger bias in both $\widehat H$ and the
induced horizon estimate when the lag law is misspecified. Table
\Cref{tab:observability_misspec} summarizes that sensitivity.
...
\begin{tabular}{lccc}
\hline
scenario & $H$ bias & $\log n^*$ bias & $\log n^*$ RMSE \\
\hline
correct & $-0.0000$ & $+0.0002$ & $0.0090$ \\
sinusoidal & $-0.0932$ & $+0.1100$ & $0.1104$ \\
piecewise & $-0.2114$ & $+0.8291$ & $0.8292$ \\
\hline
\end{tabular}
\caption[Misspecification probe for observability]{Misspecification probe for
observability.}
\label{tab:observability_misspec}
\end{table}
