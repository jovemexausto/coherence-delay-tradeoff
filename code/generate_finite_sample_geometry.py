...
from experiments.core.sinkhorn import debiased_sinkhorn_divergence
...
def exact_empirical_w2(x: np.ndarray, y: np.ndarray) -> float:
    ...
    return float(np.sqrt(np.mean(cost[row_ind, col_ind])))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tables-dir",
        type=Path,
        default=Path("artifacts/tables/gaussian"),
    )
