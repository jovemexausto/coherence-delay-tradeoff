...
def dirac_Hölder_staleness(zeta: float, H: float, n: int) -> float:
    return math.sqrt(sum((zeta * (j**H)) ** 2 for j in range(n)) / n)


class HölderStalenessBoundTest(unittest.TestCase):
    def test_dirac_path_saturates_averaged_coupling_bound(self) -> None:
        zeta = 0.7
        for H in (0.35, 0.5, 0.75, 1.0):
            for n in (8, 16, 32, 64):
                exact_w2 = dirac_Hölder_staleness(zeta, H, n)
                coupling_bound = math.sqrt(
                    sum((zeta * (j**H)) ** 2 for j in range(n)) / n
                )
                self.assertAlmostEqual(exact_w2, coupling_bound, places=12)

    def test_Hölder_staleness_has_n_to_the_h_scaling(self) -> None:
        zeta = 1.0
        for H in (0.35, 0.5, 0.75, 1.0):
            values = [dirac_Hölder_staleness(zeta, H, n) for n in (32, 64, 128, 256)]
    ...
    def test_hypothetical_linear_average_bound_fails_for_dirac_lipschitz_path(
        ...
        zeta = 1.0
        n = 64
        exact_w2 = dirac_Hölder_staleness(zeta, 1.0, n)
